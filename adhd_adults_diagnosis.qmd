...
ggplot2::ggplot(
  ...
  labs(
    ...
    alt = "Grid of scatter plots showing the sensitivity and specificity of various adult ADHD diagnostic tools, distinguishing between studies with and without neurotypical control groups."
  ) +
  xlim(c(0, 100)) + 
  ylim(c(0, 100))
...
for (name_1 in unique(d_ss_long$name)) {
  plot <- ggplot2::ggplot(
    ...
    labs(
      shape = "Neurotypical only",
      alt = paste("Scatter plot showing the sensitivity and specificity of the", name_1, "diagnostic tool for adult ADHD, comparing studies with and without neurotypical control groups.")
    ) +
    ggtitle(name_1) +
    xlim(c(0, 100)) + 
    ylim(c(0, 100))
