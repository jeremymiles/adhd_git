...
plot_diagnostic_meta <- 
  function(
    ...
    dat_sens$calculated = inv_logit(dat_sens$yi)
    ...
    dat_spec$calculated = inv_logit(dat_spec$yi)
    ...
    dat_long %>%
      ...
      dplyr::select(ID, starts_with("sens"), starts_with("spec")) %>%
      knitr::kable() %>%
      print()
    ...
    cat(glue::glue("**I-squared for Specificity:** {i2_results[1]}%\n\n"))    
    cat(glue::glue("**I-squared for Specificity:** {i2_results[2]}%\n\n"))    
    
    # 3. Setup Plotting Layout
    all_studies <- dat_long %>%
      distinct(study) %>%
      # Add the instrument label early so we can sort by it
      mutate(instrument_label = data[[instrument_col]][match(study, data$study)]) %>%
