...
plot_diagnostic_meta <- 
  function(
    ...
    res <- metafor::rma.mv(yi, vi, mods = ~ outcome - 1, 
                           random = ~ outcome | study,
                           struct = "UN", data = dat_long)
    
    i2_results <- get_i2_un(res)
    cat(glue::glue(
      "**I-squared for Specificity:** {i2_results[1]}%<p>")
    )    
    cat(glue::glue(
    ...
    if (length(unique(dat_long$neurotypical_mod)) == 1) {
      ...
      cat("Neurotypical analysis coming.<p>")
      #print(res_neurotypical)
      ...
      anova(res_neurotypical, L = c(-1, 1, 0, 0)) %>% print()
      cat("Test of difference in specificity for neurotypical vs not<p>")
      anova(res_neurotypical, L = c(0, 0, -1, 1)) %>% print()
