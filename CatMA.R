...
CatMA <- function(
  ...
  if (nrow(d_es) > 0) {
    ...
    cat(
      ...
      Intervention sample =  {sum(d_es$ai + d_es$bi, na.rm = TRUE)} <p>
      Control sample = {sum(d_es$ci + d_es$di, na.rm = TRUE)}  <p>
      Total sample = {sum(d_es$ai + d_es$bi, na.rm = TRUE) + sum(d_es$ci + d_es$di, na.rm = TRUE)}. <p>
      "
      )
    )
